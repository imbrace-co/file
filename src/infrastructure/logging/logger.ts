 ...
 *   logger.info("File_uploaded", { function: "uploadFile", entity: "EMPTY" });
 *
...
// Deployment tier for log filtering (Loki `{env="dev"}`). Reads the service's
// native env var; normalized so the value is always a clean dev/staging/prodv2
// regardless of the raw spelling. DEPLOY_ENV is an explicit override.
...
const canonicalFormat = winston.format.printf((info) => {
  ...
  // Keep any extra structured fields the caller passed (e.g. file_id).
...
// Verbose on every non-prod tier; preserves file-service's prior dev-debug
// behavior now that the env value is normalized ("development" -> "dev").
const logger = winston.createLogger({
  level: ENV === "prodv2" ? "warn" : "debug",
  levels,
  transports,
});
